...
#define _PRINT_LOG_(log_level, str_format, ...)                                       \
    if (APP_LOG_LEVEL >= log_level) {                                                 \
        ...
        printf("%d-%02d-%02d %02d:%02d:%02d [%d] [%02d] " str_format,                 \
            ltm.tm_year + 1900, ltm.tm_mon + 1, ltm.tm_mday,                          \
            ltm.tm_hour, ltm.tm_min, ltm.tm_sec, getpid(), log_level, ##__VA_ARGS__); \
    \
}
...
{
    if (data[length] != '\0' /* check is needed to skp static strings */)
        data[length] = '\0'; // this is needed for log to write data out
...
void web(int socketfd)
{
    ...
    if (setsockopt(socketfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) < 0) {
        ERROR_LOG("Error setting socket receive timeout.");
    }
    ...
    int exitCode = 100;
    char http_method[16];
    char if_modified_since[32]; // format Wed, 21 Oct 2015 07:28:00 GMT
    char last_modified[32]; // format Wed, 21 Oct 2015 07:28:00 GMT
    char buffer[BUFSIZE + 1];
    struct stat request_file_stat;
    struct tm* request_file_gmt;

    for (refresh_flag = tail_flag = request_count = 0; KEEP_ALIVE; request_count++, refresh_flag = tail_flag = 0) {

        {
            len = read(socketfd, http_method, 4); // read 1st 4 chars
            if (len < 1) {
                exitCode = 101;
            ...
            if (strncmp(toUpper(http_method), "GET ", 4)) {
                send_error(socketfd, FORBIDDEN, "Only GET is supported", http_method);
                sleep(5); // avoid spam
                exitCode = 102;
        ...
        {
            len = read(socketfd, buffer, 2050); // read max get request size 2048
            if (len < 0 || len > 2048) {
            ...
            char* found = strcasestr(buffer, "If-Modified-Since:");
            if_modified_since[0] = '\0';
            if (found > 0) {
                sscanf(&found[18] /* skipping If-Modified-Since: */, "%[^\n]s", if_modified_since);
                trim((char*)if_modified_since);
            }
        }
        ...
        { // keep only url
            ...
            // read refresh flag on url
            char* found_params = strstr(buffer, "?");
            if (found_params) {
                refresh_flag = strstr(found_params, "refresh") ? 1 : 0;
                tail_flag = strstr(found_params, "tail") ? 1 : 0;
                *found_params = '\0'; // terminate buffer at ?
            }
            DEBUG_LOG("Flags, refresh(%d), tail(%d)\n", refresh_flag, tail_flag);
        }
        ...
        if (stat(buffer, &request_file_stat) == 0) { /* file found */
            ...
            request_file_gmt = gmtime(&request_file_stat.st_mtim.tv_sec);
            ...
            if (request_file_stat.st_mode & S_IFDIR) { /* directory */
            ...
            else if (!refresh_flag && strcmp(last_modified, if_modified_since) == 0) {
                ...
                long content_len = (long)lseek(file_fd, (off_t)0, SEEK_END); /* lseek to the file end to find the length */
                lseek(file_fd, (off_t)0, SEEK_SET); /* lseek to the file start */

                if (content_len > BUFSIZE && tail_flag) {
                    lseek(file_fd, -BUFSIZE, SEEK_END);
                    content_len = BUFSIZE; /* lseek to the last BUFSIZE */
                }
                len = sprintf(buffer, "HTTP/1.1 200 \nServer: nweb\nContent-Length: %ld\nCache-control: private\nLast-Modified: %s\n\n", content_len, last_modified);
                write_data(socketfd, buffer, len, DEBUG_LEVEL);
                ...
                while ((len = read(file_fd, buffer, BUFSIZE)) > 0) {
                    ...
                    // break, even if there is more data to be read, client must execute a fetch again
                    ...
                    nwritten += len;
                    if (nwritten >= content_len || tail_flag)
                        break;
                }
                close(file_fd);
            ...
            if (*if_modified_since) { // never resend icon
                write_data(socketfd, "HTTP/1.1 304\n\n", 14, DEBUG_LEVEL);
            }
    ...
    DEBUG_LOG("Fork exit code %d, served %d requests\n", exitCode, request_count);
...
int main(int argc, char** argv)
{
    ...
    if ((n = chroot(argv[2])) != 0) {
        WARN_LOG("WARN: Can't root to directory %s, errno %d\n", argv[2], n);
    }
